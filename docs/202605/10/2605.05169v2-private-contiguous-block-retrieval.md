...
tldr: "本研究提出了私有连续块检索（PCBR）问题，旨在从N个非共谋服务器中私密检索D个连续索引的消息。针对存储和流媒体系统中文件分段的需求，研究探讨了利用连续结构提高效率的可能性。通过建立平衡{0,1}-线性方案的检索率上限和子分组级别下限，本文构建了一种在保持最优检索率的同时显著降低子分组复杂度的方案。"
source: arxiv
selection_source: fresh_fetch
motivation: 针对存储和流媒体系统中文件分段检索的隐私需求，探索利用消息索引的连续性结构来优化私有信息检索的效率。
method: "研究了平衡{0,1}-线性方案，推导了检索率上限和子分组级别下限，并设计了一种利用连续块结构的速率最优方案。"
result: 确定了PCBR的最优检索率，并构造了一个在广泛参数范围内达到该速率且子分组级别匹配下限的方案。
conclusion: 证明了利用连续块结构可以在不损失检索率的前提下，比传统多消息PIR方案更有效地降低子分组复杂度。
---

## 摘要
我们引入了隐私连续块检索（Private Contiguous-Block Retrieval, PCBR）问题。在该问题中，用户从存储在 $N$ 个非共谋服务器上的 $K$ 条副本消息中检索一个具有连续索引的 $D$ 条消息块，同时向每个服务器隐藏所请求块的身份。该问题的动机源于存储和流媒体系统，其中文件被分割成有序的段。与多消息隐私信息检索（MPIR）不同（在 MPIR 中可以请求任何 $D$ 子集），PCBR 将需求族限制为连续块。这种放宽提出了一个自然的问题：是否可以利用这种结构来提高检索效率？我们针对平衡 $\{0,1\}$-线性方案回答了这个问题。我们为所有问题参数建立了可实现检索率的上界，推导了任何达到该检索率上界的方案所需的子分组级别（subpacketization level）的下界，并构建了一个在广泛的问题参数范围内其子分组级别与下界相匹配的率最优方案。尽管最优 PCBR 率与已知的最佳 MPIR 率逆界（converse bound）一致，但现有的 MPIR 方案对于 PCBR 可能是次优的，并且可能需要大得多的子分组级别。相比之下，我们的方案利用连续块结构，以降低的子分组级别实现了最优率。
