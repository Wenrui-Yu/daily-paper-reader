...
tldr: "本文提出私有结构化子集检索（PSSR）问题，研究在需求受限于特定结构族时，如何从多个非共谋服务器中私密检索多个消息。该研究推广了多消息PIR，针对平衡{0,1}-线性方案推导了检索率与分包水平的理论界限，并提出了通用的优化构建框架。结果表明，利用需求结构能显著提升检索效率并降低分包复杂度，在特定场景下优于传统MPIR方案。"
source: arxiv
selection_source: fresh_fetch
motivation: 旨在解决传统私有信息检索方案未考虑实际应用中检索需求往往具有特定结构限制的问题。
method: "针对平衡{0,1}-线性方案推导了检索率与分包水平的逆界，并开发了基于优化的通用方案构建框架。"
result: 发现利用需求结构可使检索率超过MPIR上限，并能在不改变最优率的情况下显著降低分包水平。
conclusion: PSSR通过挖掘需求空间的结构特征，为实现高效且低复杂度的私有数据检索提供了新的理论路径。
---
