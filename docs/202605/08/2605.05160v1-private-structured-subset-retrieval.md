...
## 摘要
我们引入了隐私结构化子集检索（PSSR）问题，其中用户从复制在 $N$ 个非共谋服务器上的 $K$ 条消息数据库中检索 $D$ 条消息，且需求被限制在一个已知的 $D$ 子集结构化族中。该公式推广了经典的隐私信息检索（PIR）和多消息 PIR（MPIR），并涵盖了需求空间受特定应用结构约束的情景。针对平衡 {0,1}-线性方案，我们推导了最大检索率和最小子分组级别的逆界，并开发了一个基于优化的框架，用于为通用结构化需求族构建方案。我们的结果表明，对于某些族，PSSR 速率逆界可以超过已知的最佳 MPIR 速率上界；当该 PSSR 界可实现时，MPIR 速率最优方案对于这些族而言变得不再最优。通过利用需求结构，我们的 PSSR 方案在许多族中实现了更高的检索率，且性能绝不低于已知的最佳平衡 {0,1}-线性 MPIR 方案。我们的结果还表明，即使在最优速率不变的情况下，需求结构也可以减少所需的子分组。我们在连续需求族上的并行工作通过产生具有显著更小子分组且无域大小限制的速率最优方案，进一步说明了该框架的应用范围，并对基于 MPIR 的方案进行了改进。
