...
tldr: 本研究针对连续分布采样中浮点数导致的舍入误差和安全隐患，提出了一种名为 Continuous-Eris 的高阶分离逻辑。该逻辑专门用于验证基于可计算实数的精确采样算法，克服了概率选择、高阶函数和动态内存分配带来的验证挑战。作者在 Rocq 证明助手中验证了均匀、高斯及拉普拉斯分布的采样器，为实现高可靠性的概率程序提供了形式化保障。
source: arxiv
selection_source: fresh_fetch
motivation: 传统的浮点数采样算法存在舍入误差和安全风险，而精确采样算法虽然可靠但实现逻辑极其复杂。
method: 提出了一种名为 Continuous-Eris 的高阶分离逻辑，用于形式化验证可计算分布的精确采样程序。
result: 在 Rocq 证明助手中成功验证了均匀、高斯、拉普拉斯分布的采样器以及一个精确实数算术库。
conclusion: 该框架为开发和验证具有严格数学正确性保证的连续分布采样算法提供了有效的形式化工具。
---

## 摘要
大多数连续分布采样算法的实现都使用浮点数，这会引入舍入误差和近似。这些误差可能难以分析，并且在用于差分隐私算法时可能会引发安全问题。一种替代方案是使用基于可计算实数的精确采样算法，这类算法可以惰性地生成具有任意精度的连续样本位。然而，这些算法非常复杂，其实现和使用涉及多种在语义上具有挑战性的语言特性，例如概率选择、高阶函数和动态分配的可变状态。在本文中，我们提出了 Continuous-Eris，这是一种用于验证可计算分布精确采样算法正确性的高阶分离逻辑。为了演示 Continuous-Eris，我们验证了均匀分布、高斯分布和拉普拉斯分布的可计算采样器，以及一个用于处理生成样本的精确实数算术库的正确性。本文中的所有结果均已在 Rocq 证明助手中得到验证。
